(() => {
  ...
  document.addEventListener('click',e=>{const b=e.target.closest('button');if(!b)return;const action=b.dataset.action,panel=b.dataset.panel;
    if(panel)openModal(panel);if(action==='play')startRun(false);if(action==='new'){if(confirm('Start a new run? Existing stage progress will reset.'))startRun(true);}if(action==='volume'){save.muted=!save.muted;persist();toast(save.muted?'SFX OFF':'SFX ON');}if(action==='resume'){state.paused=false;state.screen='game';state.runStamp=performance.now();showOnly(null);$('#hud').classList.remove('hidden');}if(action==='restart-stage'){save.lives=3;state.paused=false;state.screen='game';state.runStamp=performance.now();showOnly(null);loadStage(state.stage,false);}if(action==='home'){commitTime();state.screen='title';showOnly('#titleScreen');$('#hud').classList.add('hidden');persist();}if(action==='close-modal')closeModal();if(action==='previous'){state.modalPage=Math.max(0,state.modalPage-1);renderModal();}if(action==='next'){state.modalPage=Math.min(1,state.modalPage+1);renderModal();}if(action==='retry-stage'){save.lives=3;save.stage=state.stage;state.screen='game';state.runStamp=performance.now();showOnly(null);$('#hud').classList.remove('hidden');loadStage(state.stage);}if(action==='save-score'){const name=($('#runnerName').value||'PLAYER').trim().toUpperCase().slice(0,12);save.scores.push({name,time:state.runBase,date:Date.now()});persist();state.screen='title';showOnly('#titleScreen');}
    if(b.dataset.biome!==undefined){state.modalPage=Number(b.dataset.biome);renderModal();}if(b.dataset.level!==undefined){const n=Number(b.dataset.level);if(n<=save.unlocked){save.lives=3;commitTime();state.screen='game';state.runStamp=performance.now();showOnly(null);$('#hud').classList.remove('hidden');loadStage(n);}}
